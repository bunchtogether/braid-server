...
class Server extends EventEmitter {
  ...
  constructor(uwsServer, websocketPattern = '/*', websocketBehavior = {
    ...
    this.receiverServers = new PublisherServerManager(); // Tracks relationships of publishing socket / key pairs to publishing regexes

    this.receiverSessions = new PublisherSessionManager(); // Tracks relationships of publishing socket / key pairs to receiving regexes

    this.publisherSessions = new PublisherSessionManager(); // Tracks relationships of publishing socket / key pairs to receiving servers

    this.publisherServers = new PublisherServerManager(); // Active (incoming) sockets
    //   Key: Socket ID
    //   Value: Socket object
    ...
    this.peerSockets = new DirectedGraphMap(); // Active (outgoing) peer connections
    //   Key: Peer ID
    ...
    this.subscriptions = new DirectedGraphMap(); // Active publishers
    //   Source: Socket ID
    //   Target: Key

    this.publishers = new DirectedGraphMap(); // Active event subscriptions
    //   Source: Socket ID
    //   Target: Event Name

    this.eventSubscriptions = new DirectedGraphMap(); // Keys without subscribers that should be flushed from data
    //   Key: key
  ...
  async _handleCredentialsRequest(socketId, credentials, newClientCredentials) {
    ...
    const unencoded = new CredentialsResponse({
  ...
  async handlePeerRequest(socketId, credentials, peerId) {
    if (this.peerConnections.has(peerId)) {
      ...
      const unencoded = new PeerResponse({
        id: this.id,
        success: false,
        code: 801,
  ...
  async handleSubscribeRequest(socketId, credentials, key) {
    ...
    const unencoded = new SubscribeResponse({
      key,
      success: response.success,
      code: response.code,
      message: response.message
  ...
  async handleEventSubscribeRequest(socketId, credentials, name) {
    ...
    const unencoded = new EventSubscribeResponse({
      name,
      success: response.success,
      code: response.code,
      message: response.message
  ...
  async handlePublishRequest(socketId, credentials, key) {
    ...
    const unencoded = new PublishResponse({
      key,
      success: response.success,
      code: response.code,
      message: response.message
  ...
  async syncPeerConnection(peerId) {
    ...
    const peerSync = new PeerSync(this.id, new PeerDump(this.peers.dump()), new ProviderDump(this.providers.dump()), new ReceiverDump(this.receivers.dump()), new ActiveProviderDump(this.activeProviders.dump()), new PeerSubscriptionDump(this.peerSubscriptions.dump()), customMapDumps, customSetDumps);
